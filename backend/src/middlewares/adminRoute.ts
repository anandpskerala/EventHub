...
export const adminRoute = async (req: CustomRequest, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        if (!decoded.roles?.includes("admin")) {
            res.status(HttpStatusCode.UNAUTHORIZED).json({ message: HttpResponse.NEED_ADMIN_ACCESS });
            return;
        }
        next();
    } catch (err) {
